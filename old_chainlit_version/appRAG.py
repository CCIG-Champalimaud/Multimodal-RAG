...
actions = [
        ...
        cl.Action(name="Retrieve Report", value="retrieve_import", description="Retrive similar reports to the one provided"),
         cl.Action(name="Retrieve Report & Images", value="retrieve_import_images", description="Retrive similar reports and images to the one provided"),
...
async def on_message(message: cl.Message):
    ...
    if similars == 0:
        ...
        # Get the context from the user
            ...
            used_context = retriever.get_relevant_documents(msg.content,search_kwargs={'k': similars})
...
async def on_action(action): 
    ...
    msg = cl.Message(
        content=f"The most similar report that I have access to is the following: \n Unique ID: {retrieved_documents[0][0].metadata['uid']} \n Content: {retrieved_documents[0][0].page_content}")
    msg.actions = actions
    await msg.send()

@cl.action_callback("Retrieve Report & Images") # Upload PDF, choose ID, and embed in the database
async def on_action(action): 
    files = None
    ...
    image = cl.Image(path=img_path, name="processed_image", display="inline")
    
    msg = cl.Message(
        content=f"The most similar report that I have access to is the following: \n Unique ID: {retrieved_documents[0][0].metadata['uid']} \n Content: {retrieved_documents[0][0].page_content} \n \
                    Found {len(retrieved_images['ids'])} images related to the report \n Images shown below: \n",
        elements=[image],
        )
    msg.actions = actions
    await msg.send()
